...
router.get("/product/:pid", async (req, res) => {
    try {
        let readfile = await readJSONfile(productPath);
        let jsonOBJ = JSON.parse(readfile).filter((item) => item.no == req.params.pid)
...
router.put("/editproduct/:pid", authorize,async (req, res) => {
    try {
        let readFile = await readJSONfile(productPath);
        let jsonOBJ = JSON.parse(readFile);
        let findObj = jsonOBJ.findIndex((item) => item.no == req.params.pid);
...
router.delete("/delproduct/:pid", authorize,async (req, res) => {
    try {
        let readfile = await readJSONfile(productPath)
        let jsonOBJ = JSON.parse(readfile);
        let itemIndex = jsonOBJ.findIndex((item) => item.no == req.params.pid);
        if (itemIndex != -1) {
